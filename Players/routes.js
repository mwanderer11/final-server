...
function playerRoutes(app) {
    ...
    const deletePlayer = async (req, res) => {
        const status = await dao.deletePlayer(req.params);
        res.json(status);
    }
    ...
    const findPlayerByName = async (req, res) => {
        const player = await dao.findPlayerByName(req.params.playerName);
        res.json(player);
    }

    const findPlayerById = async (req, res) => {
        const player = await dao.findPlayerById(req.params);
        res.json(player);
    }
